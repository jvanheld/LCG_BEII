...
4. Compute and draw the Lorenz curves for the ChIP-seq data and for the genomic input. 
5. Compute various scores that might be used to compare the counts per bin between ChIP-ped and input datasets. 
6. Based on the Poisson distribution, compute a value per bin, as well as an adjusted p-value (corrected for multiple testing). 
7. Draw a volcano plot with the log2-fold-change as measure of the effect size (abscissa) and the $-log_{10}(p_{adj})$ on the ordinate.
8. Select the significant bins.
9. Load the results in IGV and compare the results with the peaks returned by a real peak-caller. 
