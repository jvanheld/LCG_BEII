...
#' @description Plot a Lorenz curve to compare the cumulative distribution of counts between two coverage proviles (e.g. bedGraph or bigWig files). 
...
#' @return a table with Lorenz statistics
#' @export
plot.lorenz <- function(chip.counts, 
  ...
  lorenz <- data.frame(
    rank=1:nbins,
    quantile=(1:nbins)/nbins,
    chip=cumsum(sort(chip.counts))/ sum(chip.counts),
    input=cumsum(sort(input.counts))/ sum(input.counts)
  )
  lorenz$diff <- lorenz$input - lorenz$chip
  
  ## Identify the range where input and chip-seq differ most
  max.diff.rank <- lorenz$rank[which.max(lorenz$diff)]
  max.diff.quantile <- lorenz$quantile[which.max(lorenz$diff)]
  
  plot(lorenz$quantile, lorenz$input, type="l", col="darkgrey",
       main="Loren curves", 
       xlab="Quantile", 
  ...
  lines(lorenz$quantile, lorenz$chip, type="l", col="darkblue")

  
  abline(v=max.diff.quantile, col="pink")  
  arrows(x0=max.diff.quantile,  
         y0=lorenz$input[max.diff.rank], 
         x1=max.diff.quantile, 
         y1=lorenz$chip[max.diff.rank], angle = 20, length = 0.1, code=3, col="red", lwd=2)  

  legend("topleft", 
         legend=c(
           "ChIP-seq", 
           "input", 
           paste("max difference =", round(digits=3, max(lorenz$diff))),
           paste("best quantile q =", round(digits=2, max.diff.quantile))
           ), 
         col=c("darkblue", "darkgrey", "red", "pink"), lwd=2)
#  return(lorenz)  
}

lorenz.table <- plot.lorenz(
